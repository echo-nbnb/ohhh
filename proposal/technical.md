...
└─────────────────────────────────────────────────────────┘
          ...
          │                   Python 本地端                   │
          │  ┌─────────────┐  ┌──────────┐  ┌────────────┐ │
          │  │  颜色提取    │  │ MediaPipe │  │  手势状态机 │ │
          │  │  (物件+衣物) │  │ 手部追踪  │  │  (6 模式)  │ │
          │  └──────┬──────┘  └────┬─────┘  └─────┬──────┘ │
          │         │              │              │         │
          │  ┌──────┴──────────────┴──────────────┴──────┐  │
          │  │             Bridge 层                      │  │
          │  │  color_detector / sketch / character     │  │
          │  └──────────────────┬────────────────────────┘  │
          │                     │                            │
          │  ┌──────────────────┴────────────────────────┐  │
          │  │         RAG 检索生成                       │  │
...
[食指绘画] ────────────────→ MediaPipe 追踪指尖
                              ...
                              (QuickDraw CNN + 颜色加权)
                                    │
                                    │ {recognized_object: "岳麓书院", confidence: 0.87}
                                    ▼
                              SketchBridge ───────────────→ [系统直接说出：物象名]


[握拳确认] ────────────────→ FSM 状态机
                                    │
                                    │ {object_confirmed: "岳麓书院"}
                                    ▼
                              [物象发光生长]
...
| `mode` | `str` | `"DRAWING"` | FSM 模式：GLOBAL/DRAWING/CANDIDATE/CHAR_RECOMMEND/CHAR_WHEEL/COLOR_EXTRACTION |
...
| 字段 | 类型 | 示例 | 说明 |
|------|------|------|------|
| `color_type` | `str` | `"岳麓绿"` | 六色基调之一 |
| `source` | `str` | `"object"` | 来源：object（物件）/ clothing（衣物） |
| `confidence` | `float` | `0.85` | 匹配置信度 |
| `fallback_triggered` | `bool` | `false` | 是否触发了衣物兜底 |
...
摄像头帧 (640×480)
      ...
      └──────────┬─────────────────────────┘
          ...
          ColorDetectorBridge
                 │
                 ▼
          {color_type, source, confidence} → Unity
```
...
```
                    ┌─────────────────────┐
                    │ COLOR_EXTRACTION   │ ← 颜色提取（第一幕专用）
                    │ AWAITING_OBJECT    │   物件分析→确认/衣物兜底
                    └─────────┬───────────┘
                              │
                    握拳确认 ──┴ ── 物件匹配失败
                              │
...
- **6 种手势识别**：握拳/张手/食指伸出/悬停/未知
- **6 模式 FSM**：GLOBAL / DRAWING / CANDIDATE / CHAR_RECOMMEND / CHAR_WHEEL / COLOR_EXTRACTION 🔸新增
...
第一幕系统主动提取用户底色：优先分析用户物件颜色，匹配失败则识别用户衣物颜色。
...
| 端口 | 通道 | 频率 | 内容 |
|------|------|------|------|
| **:8888** | 主通道 | 事件驱动 | 物象识别结果、推荐人物、手势状态、确认/拒绝 |
...
| 消息 | 内容 |
|------|------|
| `object_recognition_confirmed` | 用户确认该物象（握拳） |
| `object_recognition_cancelled` | 用户取消（张开手） |
...
> 颜色提取（物件+衣物）实现后（预计3-5天工作量），视觉输入模块可达 100%，整体完成度 ~94%。
