...
import "./DualColorLiquidChrome.css";
...
export default function DualColorLiquidChrome({ colorA = "#F2E700", colorB = "#355BFF", speed = 0.28, amplitude = 0.42, interactive = false }) {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      uniform float uTime; uniform vec3 uResolution; uniform vec3 uColorA; uniform vec3 uColorB; uniform float uAmplitude; uniform vec2 uMouse; varying vec2 vUv;
      ...
      vec2 warp(vec2 uv) { vec2 p = uv; for (float i = 1.0; i < 9.0; i++) { p.x += uAmplitude / i * cos(i * 3.4 * p.y + uTime * 1.1 + uMouse.x * 2.2); p.y += uAmplitude / i * sin(i * 2.8 * p.x + uTime * 0.9 + uMouse.y * 2.2); } return p; }
      vec4 renderImage(vec2 uvCoord) {
        vec2 fragCoord = uvCoord * uResolution.xy; vec2 uv = (2.0 * fragCoord - uResolution.xy) / min(uResolution.x, uResolution.y);
        vec2 p = warp(uv); vec2 diff = uvCoord - uMouse; float dist = length(diff); float ripple = sin(12.0 * dist - uTime * 2.2) * 0.035; float falloff = exp(-dist * 18.0); p += normalize(diff + 0.0001) * ripple * falloff;
        float angle = atan(p.y, p.x); float radius = length(p); float flowA = sin(p.x * 3.8 + p.y * 2.2 + uTime * 0.9); float flowB = cos(p.y * 4.6 - p.x * 2.4 - uTime * 0.75); float spiral = sin(angle * 2.2 + radius * 8.0 - uTime * 1.25); float n = noise(p * 1.8 + uTime * 0.08);
        float mixer = flowA * 0.42 + flowB * 0.32 + spiral * 0.34 + n * 0.34; mixer = smoothstep(-0.42, 0.52, mixer); vec3 color = mix(uColorA, uColorB, mixer);
        float whiteBandA = abs(sin(p.x * 4.8 + p.y * 3.6 + uTime * 0.72)); float whiteBandB = abs(cos(p.x * 2.4 - p.y * 5.2 - uTime * 0.55)); float whiteMask = smoothstep(0.76, 0.98, whiteBandA * whiteBandB); color = mix(color, vec3(1.0), whiteMask * 0.82);
        float chrome = 0.72 + 0.32 * sin(p.x * 7.0 + p.y * 3.0 + uTime); chrome += 0.18 * cos(radius * 12.0 - uTime * 1.5); color *= chrome; color = pow(color, vec3(0.78)); color = clamp(color, 0.0, 1.0); return vec4(color, 1.0);
      }
  ...
  }, [colorA, colorB, speed, amplitude, interactive]);

  return <div ref={containerRef} className="dualLiquidChrome" />;
}
